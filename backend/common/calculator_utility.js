...
var getOptionReturn = async function(premium, collateral) {
    let option_return = premium / collateral * 100;
    option_return = Number(option_return.toFixed(option_decimal_places));
    return option_return;
}

var getOptionARRReturn = async function(date_opened, date_of_expiry, premium, collateral) {
    const num_days = await date_utility.getNumberDays(date_opened, date_of_expiry);
    const option_return  = await getOptionReturn(premium, collateral);
